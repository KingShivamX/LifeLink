...
router.post('/', [
  ...
  body('patient.medicalRecordNumber').optional().trim(),
  body('requester.name').trim().isLength({ min: 1 }),
  body('requester.relationship').optional().trim(),
  body('requester.email').optional().isEmail().normalizeEmail(),
  body('requester.phone').optional().trim(),
  body('medicalInfo.hospital.name').trim().isLength({ min: 1 }),
  body('medicalInfo.hospital.address').trim().isLength({ min: 1 }),
  body('medicalInfo.hospital.contactNumber').optional().trim(),
  body('medicalInfo.doctor.name').trim().isLength({ min: 1 }),
  body('medicalInfo.diagnosis').optional().trim(),
