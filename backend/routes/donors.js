...
router.get('/', [
  ...
  try {
    ...
    let donorsQuery = Donor.find(query)
      .select('-password -verificationToken -medicalConditions -medications')
...
router.post('/', [
  body('firstName').trim().isLength({ min: 1, max: 50 }).withMessage('First name is required and must be under 50 characters'),
  body('lastName').trim().isLength({ min: 1, max: 50 }).withMessage('Last name is required and must be under 50 characters'),
  ...
  body('weight').isFloat({ min: 50, max: 200 }).withMessage('Weight must be between 50-200 kg'),
  ...
  try {
    ...
    Object.keys(req.body).forEach(key => {
      if (key === 'address' && req.body.address) {
        donor.address = { ...donor.address.toObject(), ...req.body.address }
...
router.put('/:id/availability', [
  ...
  body('emergencyOnly').optional().isBoolean().withMessage('Emergency only must be boolean'),
  ...
  try {
    ...
    if (schedule) {
      donor.availability.schedule = { ...donor.availability.schedule, ...schedule }
    }
    donor.lastActive = new Date()
...
router.post('/search/nearby', [
  body('bloodType').isIn(['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-']).withMessage('Valid blood type is required'),
  body('coordinates').isArray({ min: 2, max: 2 }).withMessage('Valid coordinates are required'),
  ...
  try {
    ...
    .select('firstName lastName bloodType phone address availability donationCount')
    .limit(20)
